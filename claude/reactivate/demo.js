...
function start(){clearTimers();mode='intro';$('#app').hidden=false;$('#app').classList.remove('depart');$('#app').querySelector('.pixel-dissolve')?.remove();$('#email').hidden=true;$('#artifactApp').hidden=true;$('#envelope').hidden=true;$('#skip').hidden=false;$('#days').textContent='1';reversing=false;paused=false;let day=1;
 function tick(){if(day<7){$('#days').textContent=++day;later(tick,Math.max(80,440-day*55))}else later(()=>{if(reduced){email();return}pixelateApp();$('#app').classList.add('depart');later(()=>{$('#app').hidden=true;$('#envelope').hidden=false;later(email,2050)},1350)},850)}
 later(tick,650);
}
